...
INSERT INTO dictionaries.namechange_reasons (reason, description)
    VALUES 
        ('original', 'The original published name'),
        ('misspelling', 'The taxon''s name was mispelled'),
...
-- Note tha there is no "authorizer" or "enterer" for a taxon because it is derived from the existence of multiple opinions
-- Note that name change issues (reassignment, misspelling) are merged with synonymy issues. Although these are conceptually distinctj
-- in the world of taxonomic opinions, they are not distinct in how the database logic must treat them.
CREATE TABLE taxa (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    permid integer NOT NULL,
    name text NOT NULL, -- see special consideration below
    authority_id integer NOT NULL, -- the authority
    rank_opinion_id integer REFERENCES rank_opinions("id") NOT NULL, -- Just because a taxon is assigned to a parent taxon does not mean that its rank is immediately below that parent. Hence, we need a separate table for rank opinions from assignment opinions.
    assignment_opinion_id integer REFERENCES assignment_opinions("id"), -- the "best" opinion for the parent taxon (e.g., family of genus)
    rename_opinion_id integer REFERENCES nomenclature_opinions("id") NOT NULL, -- the "best" opinion for name/spelling
    description_opinion_id integer REFERENCES descriptions("id"), -- the "best" description. I am leaving this table undescribed for now because it touches on the PBOT system
...
-- This is for table for opinions changing the taxonomic rank (e.g., family of genus) of a taxon
 CREATE TABLE rank_opinions (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    permid integer NOT NULL,
    authorizer_person_id integer REFERENCES persons("id") NOT NULL, -- Whoever was the authorizer of the enterer
    enterer_person_id integer REFERENCES persons("id") NOT NULL, -- Whoever made the entry or edit
    taxon_id integer REFERENCES taxa("id") NOT NULL, -- cannot conflict with assignment opinion (e.g., cannot be ranked as family if assigned to a family)
    rank integer REFERENCES dictionaries.taxonomy_ranks("id") NOT NULL,
...
-- This is for table for opinions changing the parent taxon of a 
CREATE TABLE assignment_opinions (
    ...
    questioned boolean NOT NULL, -- whether the assignment is questioned or not (i.e., incertae sedis)
    evidence boolean NOT NULL, -- stated with or without evidence, see special consideration below
    reference_id integer REFERENCES refs("id"),
    preceded_by_id integer REFERENCES taxa("id"),
    succeeded_by_id integer REFERENCES taxa("id")
...
CREATE TABLE rename_opinions (
    ...
    priority_taxon_id integer REFERENCES taxa("id"), -- if the name change is due to synonymy, this is the taxon that has priority and whose name is being taken
    objective boolean, -- whether the name change is objective or subjective, REQUIRED IF reason is synonymy
    evidence boolean NOT NULL, -- stated with or without evidence
    reference_id integer REFERENCES refs("id"),
    preceded_by_id integer REFERENCES taxa("id"),
    succeeded_by_id integer REFERENCES taxa("id")
    );
